      ...
      <section class="book">
        <div class="book-box">
          ...
          <div
            ...
            class="book-item" 
            v-for="(item, index) in $store.state.arTitleMenu" 
            :key="index">
...
<script>
import { addBook, getBookList, getConentList, addContent  } from '@/api/getData'
...
export default {
  ...
  methods: {
    ...
    async getConentListAuto() {
      try{
        ...
        if(res.data.code === 200) {
          ...
          console.log(arTitleMenu)
          this.$store.commit('UpdataArTitleMenu', arTitleMenu)
          console.log(this.$store.state.arTitleMenu)
    ...
    async addContentHandle(render, title) {
      let that = this
      try{
        ...
        console.log(res)
        this.$store.commit('getNewCOntentId', res.data.data._id)
        if(res.data.code === 200) {
           this.getConentListAuto()
           this.$router.push({
